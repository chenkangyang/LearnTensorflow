...
xs = tf.placeholder(tf.float32, [None, 784]) # 8*8
ys = tf.placeholder(tf.float32, [None, 10])
x_image = tf.reshape(xs,[-1,28,28,1]) # [n_samples, 28,28,1] -1 具体是多少由导入数据决定（多少张图片）
print(x_image.shape) 

## conv1 layer ##
W_conv1 = weight_variable([5,5,1,32]) # patch: 5*5, in size 1(通道数，image的厚度), out size 32（feature_map数量，一个卷积核生成一个feature_map）
